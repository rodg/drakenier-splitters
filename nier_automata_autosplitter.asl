...
state("NieRAutomata", "BAGS")
{
	bool isWorldLoaded: 0xfe7f20;
	byte playerNameSetStatus: 0x12ce5d4;
	string32 currentCutscene: 0x17FD648, 0x294;
	bool isCutscenePlaying: 0xfa54e8;
	bool isLoading: 0x11435c0;
}

state("NieRAutomata", "1.02")
{
	bool isWorldLoaded: 0xF6E240;
	byte playerNameSetStatus: 0x124DE4C;
	string32 currentCutscene: 0x17805F0, 0x294;
	bool isCutscenePlaying: 0x102A244;
	bool isLoading: 0x14005F4;
}
...
startup
{
	vars.CutscenesHS = new HashSet<string>
	{
		/// Ending A Cutscenes
...
start 
{
	if (current.playerNameSetStatus == 1)
	{
		// Thanks Kate for this auto-start logic
		return current.isWorldLoaded && current.isWorldLoaded != old.isWorldLoaded;
	}
}

split
{
	return current.isCutscenePlaying && !old.isCutscenePlaying && vars.CutscenesHS.Contains(current.currentCutscene);
}
